function plotRaster_AuFreq(sessionData,trialData,spikeData,cell_IDs,params)
...
figure; set(gcf,'units','normalized','Position',[0.1 0.4 nNeurons*0.15+0.05 0.4],'color','w'); hold all;
...
for iNeuron = 1:nNeurons %Loop over all neurons:
    ...
    if any(cell_idx)
        ...
        splits          = {};
        splits{1}       = ismember(temptrialData.trialType,{'Y'}) & temptrialData.audioFreqChangeNorm==3 & temptrialData.audioFreqPostChangeNorm==1;
        splits{2}       = ismember(temptrialData.trialType,{'Y'}) & temptrialData.audioFreqChangeNorm==3 & temptrialData.audioFreqPostChangeNorm==2;
        ...
        subplot(3,nNeurons,iNeuron); hold all;
        for iSplit = 1:params.nSplits %Store the mean response for each of these splits
            %                         handles(iSplit) = plot(params.xtime,ratemat(iSplit,:),'-','Color',params.colors_ztrials{iSplit},'LineWidth',3);
            handles(iSplit) = plot(params.xtime,ratemat(iSplit,:),'-','Color',params.colors_splits{iSplit},'LineWidth',1.5);
...
%             subplot(2,nNeurons,iNeuron+nNeurons); hold all;
            subplot(3,nNeurons,iNeuron+nNeurons*i); hold all;
...
%                 line([vecTimes(:)';vecTimes(:)'],[intTrial*ones(1,numel(vecTimes))-1;intTrial*ones(1,numel(vecTimes))+1],'Color',params.colors_experiments{iNeuron},'LineWidth',1);
                line([vecTimes(:)';vecTimes(:)'],[intTrial*ones(1,numel(vecTimes))-1;intTrial*ones(1,numel(vecTimes))+1],'Color',params.colors_splits{i},'LineWidth',1);
...
if isfield(params,'exportfig') && params.exportfig
    export_fig(fullfile(params.savedir,'PSTH_Tuning_ExNeurons'),'-eps','-nocrop')
