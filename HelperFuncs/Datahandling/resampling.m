...
if num_chunks>4
    ...
    t_or=0:1/f1:(length(sig)-1)/f1;%1/f1:1/f1:length(sig)/f1;
    if tail_length==0
        tail_length=20000;
    end
    ...
    for i=1:length(sig_)
        sig_app=sig_{i};
        t=0:1/f1:(length(sig_app)-1)/f1;%1/f1:1/f1:length(sig_app)/f1;
        pp=pchip(t,sig_app);
        t2=0:1/f2:max(t);%1/f2:1/f2:max(t);
        clear t;
        sig_{i}=ppval(pp,t2);
        clear t2;
    end
    t=0:1/f1:(length(sig_end)-1)/f1;%1/f1:1/f1:length(sig_end)/f1;
    t2=0:1/f2:max(t);%1/f2:1/f2:max(t);
    pp=pchip(t,sig_end);
    sig_end=ppval(pp,t2);
    index=1;
...
if visualize==1
    ...
    plot(t_or,sig)
    hold on
    t_new=0:1/f2:(length(sig)-1)/f2;%1/f2:1/f2:length(sig)/f2;
    plot(t_new,sig,'r');
