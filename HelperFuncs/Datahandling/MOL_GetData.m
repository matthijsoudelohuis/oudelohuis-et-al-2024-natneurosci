...
% Datatypes         = cell array with the requested dataypes {'sessionData' 'trialData' 'spikeData' 'lfpData' 'videoData'}
% Output:
...
%% Initialize types of data: (DataTypes   = {'sessionData' 'trialData' 'spikeData' 'lfpData' 'videoData'};
        ...
        for iMouse = 1:length(MouseFolders)
            ...
            for iSes = 1:length(SessionFolders)
                ...
                for iDt = 1:length(Datatypes)
                    ...
                    if strcmp(Datatypes{iDt},'sessionData')
                        if ~isfield(sessionData,'Rec_datetime_new'); sessionData.Rec_datetime_new = cell(1,1); end
                        sessionData.Rec_datetime_new(end,1) = SessionFolders(iSes);
                    end
...
%% Correct loading multiple experiments from one session with numerical subscripts:
for i = 1:10
    sessionData.Experiment      = strrep(sessionData.Experiment,num2str(i),'');
end

%% Print output:
nMice           = numel(unique(sessionData.mousename));
nSessions       = size(sessionData.session_ID,1);
if exist('trialData','var') && isfield(trialData,'session_ID')
    nTrials         =  size(trialData.session_ID,1);
else nTrials = 0; %#ok<*SEPEX>
end
